import json
import os
os.environ["CUDA_VISIBLE_DEVICES"] = "3"  # Set to the GPU you want to use, or -1 for CPU
import sys
...
def get_model_from_run(run_path, epoch, step=-1, only_conf=False):
    ...
    build_model_func = load_function_from_runpath(
        ...
        # run_path, "models_acrobot_new.py", "build_model"  # for new acrobot model with more layers and heads #4/19/2026 no state head ablation
        run_path, "models_acrobot_new_nostatehead.py", "build_model"  # for new acrobot model with more layers and heads, no state head #4/19/2026 no state head ablation
        # run_path, "models_cartpole_finetune_acrobot.py", "build_model"  # for finetuning cartpole model with acrobot data
    )
    if build_model_func is None:
        model = models.build_model(conf.model)
        print("Using default build_model function from models.py")
    else:
        model = build_model_func(conf.model)
        print("Using custom build_model function from run's models.py")
    ...
    if step == -1:
        ...
        state = torch.load(model_path, map_location="cpu")  # ebonye 2/27/2025 added map_location="cpu" to avoid GPU memory issues
        state_dict = state["model_state_dict"]
