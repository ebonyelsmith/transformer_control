...
def train_step(model, xs, ys, optimizer, loss_func, i, args, numtrainingsteps, b=0.5, g=9.81):
    ...
    ys_scaled = ys / torch.tensor([control_scale, 1.0], device=ys.device)  # scale control and keep switch label the same

    # mask for where label is -1 for passive zero-dynamics timesteps, we don't want to compute loss on control predictions
    zero_dyn_mask = ys_scaled[..., 1] == -1  # Assuming switch label is at index 1
    zero_dyn_mask = zero_dyn_mask.unsqueeze(-1)  # Add an extra dimension for broadcasting
    ...
    active_mask = (~zero_dyn_mask[:, :-1].squeeze(-1)).float().to(raw_mse.device)  # Mask to zero out loss for zero-dynamics timesteps
    loss_controls = (raw_mse * active_mask).sum() / (active_mask.sum()+1e-8)
    # Average over non-zero-dynamics timesteps
    # loss_controls = loss_controls * (~zero_dyn_mask.squeeze(-1))  # Zero out loss for zero-dynamics timesteps
    ...
    # Need to now predict 3 labels: -1 -> 0: zero-dynamics, 0 -> 1: swing up, 1 -> 2: stabilization. So we can use cross-entropy loss instead of binary classification loss
    target_modes = (ys_scaled[:, :-1, 1] + 1).long()  # Convert -1, 0, 1 to 0, 1, 2
    mode_logits_flat = switch_logits[:, :-1, :].reshape(-1, 3)  # Reshape to (batch_size * seq_len, num_classes)
    target_modes_flat = target_modes.reshape(-1)  # Flatten the target modes
...
class SegmentedCartpoleDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        prob_zero_dyn = 0.5
        if random.random() < prob_zero_dyn:
            # choose start index from the first 50 timesteps to increase likelihood of zero-dynamics
            start_idx = random.randint(0, min(50, total_len - self.window_size - 1))
        else:
            # start_idx = random.randint(0, total_len - self.window_size - 1)
            low = min(50, total_len - self.window_size - 1)
            high = max(low+1, total_len - self.window_size - 1)
            start_idx = random.randint(low, high)

        xs_seg = xs[start_idx:start_idx + self.window_size]
        ys_seg = ys[start_idx:start_idx + self.window_size]

        return xs_seg, ys_seg, cartmass, polemass, polelength
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        with tqdm(total=num_chunks-chunk_to_resume, desc="Chunk Progress") as chunk_pbar: ###ebonye 150
            for chunk_idx in range(chunk_to_resume, num_chunks):
                ...
                #     all_cartmasses.append(torch.tensor(cartmass, device=xs.device))
                #     all_polemasses.append(torch.tensor(polemass, device=xs.device))
                #     all_polelengths.append(torch.tensor(polelength, device=xs.device))
                ...
                xs_tensor = xs_tensor_updated[mask] #.cpu()
                ys_tensor = ys_tensor[mask] #.cpu()

                cartmasses_tensor = cartmasses_tensor[mask] #.cpu()
                polemasses_tensor = polemasses_tensor[mask] #.cpu()
                polelengths_tensor = polelengths_tensor[mask] #.cpu()
                # print(f"xs_tensor device: {xs_tensor.device}")
                # print(f"ys_tensor device: {ys_tensor.device}")
                # print(f"cartmasses_tensor device: {cartmasses_tensor.device}")
                # print(f"polemasses_tensor device: {polemasses_tensor.device}")
                # print(f"polelengths_tensor device: {polelengths_tensor.device}")
...
def main(args):
    ...
    #     checkpoint_path = os.path.join(args.out_dir, f"checkpoint_epoch1_step150000.pt")
...
if __name__ == "__main__":
    ...
    # local_rank = int(os.getenv('LOCAL_RANK', '0'))
    if not args.test_run: #and local_rank == 0:
