...
def run_inference_on_model(model, XData, YS, total_time, device, dt=0.01, context=1, start_index=1, cartmass=1.0, polemass=0.1, polelength=0.5):
    ...
    # YS_context : (u1, m1), ..., (uk-1, mk-1)

    # Changed for RNN [(s1, m0, u0), ..., (sk, mk-1, uk-1)] with m0 = -1, u0 = 0.0
    ys0 = torch.cat((torch.zeros((1, 1), device=YS_context.device),
                       torch.zeros((1, 1), device=YS_context.device) - 1), dim=-1)
    if context == 1:
        YS_context = ys0 # YS_context is empty if context == 1
    else:
        YS_context = torch.cat((ys0, YS_context), dim=0)
    # YS_context : (u0, m0), (u1, m1) ..., (uk-1, mk-1)
    ...
    for i in range(start_index, n_steps):
        with torch.no_grad():
            # k = context, n = context - start_index + 1
            # actually, start_index = context, so n=1 always lol.

            # xs : last k steps in trajectory (xn, x.n, tn, t.n), ..., (xn+k, x.n+k, tn+k, t.n+k)
            # ys : last k steps in trajectory (un-1, mn-1), ..., (un+k-1, mn+k-1)
            ...
            # xs : (xn, x.n, cos tn, sin tn, t.n), ..., (xn+k-1, x.n+k-1, cos tn+k, sin tn+k, t.n+k) normalized
            # ys : (un-1, mn-1), ..., (un+k-2, mn+k-2) normalized and shifted
        ...
        # For n = 1:
        # YS_context : (u0, m0), ..., (uk, mk)
        # YS_final : (u0, m0), ..., (uk, mk)

        if YS_context.shape[0] > context:
