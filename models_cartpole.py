...
class TransformerModel(nn.Module):
    ...
    def forward(self, xs, ys=None, inds=None, inf="no"):
        ...
        if inf == "yes":
            ...
            # xs_b = xs_b / torch.tensor(states_scale, device=xs_b.device)
            # ys_b = ys_b / torch.tensor([control_scale, 1.0], device=ys_b.device)  # scale control and keep switch label the same
        ...
        # xs = xs / torch.tensor(states_scale, device=xs.device)
        # ys = ys / torch.tensor([control_scale, 1.0], device=ys.device)  # scale control and keep switch label the same
