...
class TransformerModel(nn.Module):
    ...
    def forward(self, xs, ys=None, inds=None, inf="no"):
        ...
        if inf == "yes":
            ...
            ys_b = ys_b.unsqueeze(-1) if ys_b.dim() == 2 else ys_b
            ...
            goal_state = torch.tensor([0.0, 0.0], device=xs_b.device)  
            ...
            stacked_inputs = torch.stack((states_embed, distance_embed, controls_embed), dim=2)
            zs = stacked_inputs.view(xs_b.shape[0], 3 * xs_b.shape[1], -1)
            ...
            control_prediction = self._control_head(output[:, ::3, :])  # Control predictions
            # switch_logits = self.switch_head(output[:, ::3, :])
            state_prediction = self._state_head(output[:, 2::3, :])  # State predictions
        ...
        ys = ys.unsqueeze(-1) if ys.dim() == 2 else ys
        
        if xs.shape[1] == ys.shape[1] + 1:
            # padding = torch.zeros((ys.shape[0], 1), device=ys.device)
            padding = torch.zeros((ys.shape[0], 1, 1), device=xs.device)  # 5/25/2025 padding for xs
        ...
        time_embed = self.time_embedding(timesteps) 
        goal_state = torch.tensor([0.0, 0.0], device=xs.device)  # Example goal state
        ...
        stacked_inputs = torch.stack((states_embed, distance_embed, controls_embed), dim=2)  # 5/25/2025 separate embeddings, add switch embedding
        # zs = stacked_inputs.view(xs.shape[0], 2 * xs.shape[1], -1) 
        zs = stacked_inputs.view(xs.shape[0], 3 * xs.shape[1], -1)
        zs = self.embed_ln(zs)  # Apply layer normalization to the combined embeddings

        output = self._backbone(inputs_embeds=zs).last_hidden_state

        control_prediction = self._control_head(output[:, ::3, :])  # Control predictions
        # switch_logits = self.switch_head(output[:, ::3, :])  # Switch predictions
        state_prediction = self._state_head(output[:, 2::3, :])  # State predictions
