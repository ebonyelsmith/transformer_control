import json
import os
os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # Set to the GPU you want to use, or -1 for CPU
import sys
...
def get_model_from_run(run_path, epoch, step=-1, only_conf=False):
    ...
    if step == -1:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load checkpoint from {model_path}. File may be corrupted.") from e
        state_dict = state["model_state_dict"]
