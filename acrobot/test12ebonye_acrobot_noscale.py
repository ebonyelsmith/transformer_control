...
model_name = "acrobot_checkpoints"  #finetuned with acrobot data
model_run_id= "15bf641c-dbc0-4f2f-b62f-fe04f568aacb" ### finetuned last 2 layers #"d9d1d44a-9942-40b9-a2d4-bfba4177f2ce" ### finetuned chkpt #"15bf641c-dbc0-4f2f-b62f-fe04f568aacb" #"c953cb49-31b2-4829-8d1e-d9e2b1c99dce" #"056764e2-f56a-4e25-8019-3ce5098c388c"
model_checkpoint_epoch = 1 #25 #59 #125 #14 #38 #60 #125
mode = 'indistr' # 'train', 'ood', 'indistr'

    

total_time = 11.2 #12 #16 #4 #5 #1.5
dt = 0.02
Num_of_context = 50 #150
Num_of_pendulums = 100 #10 #200 #1 #10 #20 #40 #10
...
def run_inference_on_model(model, XData, YS, total_time, device, dt=0.01, context=1, start_index=1, l1=1.0, l2=1.0, m1=1.0, m2=1.0):
    ...
    for i in range(start_index, n_steps):
        with torch.no_grad():
            ...
            cos_theta1 = torch.cos(xs[:, 0]) #9/16/2025
            sin_theta1 = torch.sin(xs[:, 0]) #9/16/2025
            cos_theta2 = torch.cos(xs[:, 1]) #9/16/2025
            sin_theta2 = torch.sin(xs[:, 1]) #9/16/2025
    ...
    theta1_model = XData_context[:, 0] #.cpu().numpy()
    theta2_model = XData_context[:, 1] #.cpu().numpy()
    thetadot1_model = XData_context[:, 2] #.cpu().numpy()
    thetadot2_model = XData_context[:, 3] #.cpu().numpy()
    # import pdb; pdb.set_trace()
...
def main(model_checkpoint_step):
    """_summary_
    """
    ...
    if mode == 'ood':
        ...
        base_dir = f"/data/esmith/Dataset_Acrobot_AIGymWithNoiseShorter_ICL/picklefolder"
    ...
    xs_cos1_tensor = torch.cos(xs[:, :, 0])  # Cosine of theta1
    xs_sin1_tensor = torch.sin(xs[:, :, 0])  # Sine of theta1
    xs_cos2_tensor = torch.cos(xs[:, :, 1])  # Cosine of theta2
    xs_sin2_tensor = torch.sin(xs[:, :, 1])  # Sine of theta2
    xs_updated = torch.cat((xs_cos1_tensor.unsqueeze(-1), xs_sin1_tensor.unsqueeze(-1), xs_cos2_tensor.unsqueeze(-1), xs_sin2_tensor.unsqueeze(-1), xs[:, :, 2:]), dim=-1)  # Update xs with cos and sin of theta1 and theta2
    ...
    return link_lengths1, link_lengths2, link_masses1, link_masses2, phase_data, controls_data, data_and_controls, pends, folder_name

for model_checkpoint_step in STEPS:
    print(f"\n{'='*80}")
    print(f"Processing checkpoint step {model_checkpoint_step}")
    print(f"{'='*80}\n")
    
    try:
        results = main(model_checkpoint_step)
        link_lengths1, link_lengths2, link_masses1, link_masses2, phase_data, controls_data, data_and_controls, pends, folder_name = results
        
        save_results_file = os.path.join(folder_name, f"results_maxcontext{Num_of_context}_numpends{Num_of_pendulums}_{mode}_alexcode.pkl")

        with open(save_results_file, "wb") as f:
            pickle.dump(results, f)

        print(f"Completed step {model_checkpoint_step}")   
    except Exception:
        print(f"Exception during step {model_checkpoint_step}, starting debugger")
        traceback.print_exc()
        ipdb.post_mortem()
