...
# import os
from eval_linearsys import get_model_from_run
...
model_name= "linearsys" #"cartpole_cos_sin_theta" #"cartpole" #"test"
model_run_id= "d0018beb-fd64-4f9c-b0f7-9d2f47735518" #"4f596a16-8d37-4d40-a757-03bcbc81a2cc"
model_checkpoint_step= 300800
...
def rk4_step(state, u, dt, l1, l2):
    """Perform a single RK4 step for the linear system."""
    def f(x, u):
        A = np.array([[l1, 0],
                      [0, l2]])
        B = np.array([[1.0],
...
def run_inference_on_model(model, XData, YS, total_time, device, dt=0.01, context=1, start_index=1, lambda1=1, lambda2=1):
    """
    ...
    Returns:
        tuple: A tuple containing:
            ...
            - x1_model (np.ndarray): Array of the first state variable (e.g., position) over time.
            - x2_model (np.ndarray): Array of the second state variable (e.g., velocity) over time.
    ...
    YS_context = YS[:context-1].to(device) if context > 1 else torch.empty((1), device = device)  #6/25/2025
    ...
    for i in range(start_index, n_steps):
        ...
        new_X = torch.tensor(next_state, dtype=torch.float32, device=device)
        ...
        new_Y = torch.tensor(u, dtype=torch.float32, device=device)
        # import pdb; pdb.set_trace()
        ...
        if counter == 0:
            ...
            # print(f"YS_context shape before: {YS_context.shape}")
            YS_context = torch.cat((YS_context, new_Y), dim=0) #6/24/2025
    ...
    x1_model = XData_context[:, 0] #.cpu().numpy()
    x2_model = XData_context[:, 1] #.cpu().numpy()
    # import pdb; pdb.set_trace()


    return T, x1_model, x2_model, YS_context
...
def main():
    ...
    contexts = [1, 2, 3, 4, 5, 6, 7, 10, 20, 30, 40]
    ...
    if mode == 'ood':
        ...
        base_dir = f"/data/esmith/Dataset_LinearSystemWithNoiseSmaller_ICL/picklefolder_test_indistr"
        ...
        base_dir = f"/data/esmith/Dataset_LinearSystemWithNoiseSmaller_ICL/picklefolder"
    ...
    with open(file_path_test_data, "rb") as f:
        xs, ys, lambda1s, lambda2s = pickle.load(f)
    ...
    pends = np.random.choice(len(xs), size=Num_of_pendulums, replace=False)  # Randomly select pendulums from all available pendulums
    ...
    lambda1s = [lambda1s[i] for i in pends]
    lambda2s = [lambda2s[i] for i in pends]
    ...
    with open(log_info_path, "w") as file:
        ...
        for lambda1, lambda2 in tqdm(zip(lambda1s, lambda2s), desc="Pendulum", total=len(lambda1s), leave=False):
            ...
            x1_rk4 = states[counter][:, 0]
            x2_rk4 = states[counter][:, 1]
            ...
            xs_dataset = np.column_stack((x1_rk4, x2_rk4))  # 7/26/2025
            ...
            for context in tqdm(contexts, desc="Context Loop", leave=False):
                ...
                T_model, x1_model, x2_model, controls_model2 = run_inference_on_model(
                    model, xs_dataset, control_values_rk4, total_time, device, dt,
                    context=context, start_index=start_index, lambda1=lambda1.item(), lambda2=lambda2.item()
                )   
                ...
                trajectory = torch.stack([x1_model, x2_model], axis=1)  # 7/26/2025
                ...
                x1_plot.append(x1_model)
                x2_plot.append(x2_model)
                ...
                state_pred_context = torch.stack([x1_model[context:], x2_model[context:]], dim=1)  # 7/26/2025
                # state_rk4_context = torch.stack([xs_dataset[context:, 0], xs_dataset[context:, 1], xs_dataset[context:, 2], xs_dataset[context:, 3]], dim=1)
                state_rk4_context = torch.stack([xs_dataset[context:, 0], xs_dataset[context:, 1]], dim=1)  # 7/26/2025
    ...
    return lambda1s, lambda2s, phase_data, controls_data, data_and_controls, pends
