...
def run_inference_on_model(model, XData, YS, total_time, device, dt=0.01, context=1, start_index=1, l1=1.0, l2=1.0, m1=1.0, m2=1.0):
    ...
    for i in range(start_index, n_steps):
        ...
        # u_unscaled = torch.sign(u) * u.pow(2) # 2/23/2026
        ...
        if counter == 0 and context > 1:
            ...
            YS_final = new_Y.unsqueeze(0) #6/24/2025
            # print(f"YS_context shape: {YS_context.shape}")
            counter = 1

            # print(YS_context.shape)
        elif counter == 0 and context == 1:
            YS_context = torch.empty((0,2), device=device)
            YS_final = new_Y.unsqueeze(0)
            counter = 1
        else:
    ...
    YS_context = YS_final
    theta1_model = XData_final[:, 0] #.cpu().numpy()
    theta2_model = XData_final[:, 1] #.cpu().numpy()
    thetadot1_model = XData_final[:, 2] #.cpu().numpy()
    thetadot2_model = XData_final[:, 3] #.cpu().numpy()
