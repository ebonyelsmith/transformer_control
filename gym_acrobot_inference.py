...
def run_single_system(link_length1, link_length2, link_mass1, link_mass2, state_data, ctrl_data, run_idx, save_dir, context=None, true_state_data=None, true_ctrl_data=None):
    ...
    plt.subplot(2, 1, 2)
    plt.scatter(range(len(actions[:, 1])), actions[:, 1], label='Control Label', color='red', s=10)
    if true_ctrl_data is not None:
        plt.scatter(range(len(true_actions[:, 1])), true_actions[:, 1], label='Reference Control Label', color='cyan', s=10, alpha=0.5)
